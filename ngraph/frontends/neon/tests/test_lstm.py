...
def check_stacked_lstm(seq_len, input_size, hidden_size,
    ...
    # concatenate weights to i, f, o, g together (in this order)
    gates = ['i', 'f', 'o', 'g']
    Wxh_neon_1 = np.concatenate([lstm_ng_1.W_input[k].value.get(None).copy().T for k in gates], 1)
    Whh_neon_1 = np.concatenate([lstm_ng_1.W_recur[k].value.get(None).copy().T for k in gates], 1)
    bh_neon_1 = np.concatenate([lstm_ng_1.b[k].value.get(None).copy() for k in gates])
    Wxh_neon_2 = np.concatenate([lstm_ng_2.W_input[k].value.get(None).copy().T for k in gates], 1)
    Whh_neon_2 = np.concatenate([lstm_ng_2.W_recur[k].value.get(None).copy().T for k in gates], 1)
    bh_neon_2 = np.concatenate([lstm_ng_2.b[k].value.get(None).copy() for k in gates])
    ...
    for i in range(num_iter):
        assert ng.testing.allclose(fprop_neon_1_list[i], fprop_ref_1_list[i], rtol=rtol, atol=atol)
        assert ng.testing.allclose(fprop_neon_2_list[i], fprop_ref_2_list[i], rtol=rtol, atol=atol)
