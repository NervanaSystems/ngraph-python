...
void create_mkldnn_innerproduct_fprop_kernel(
        ...
        int src_dims, int weights_dims, int bias_dims, int dst_dims,
        int* src_sizes, int* weights_sizes, int* bias_sizes, int* dst_sizes,
        mkldnn_primitive_desc_t src_pd,
        mkldnn_primitive_desc_t weights_pd,
    ...
    MKL_CHECK(mkldnn_memory_desc_init(&src_md, src_dims,
                                      src_sizes, data_type, mkldnn_any));
    MKL_CHECK(mkldnn_memory_desc_init(&weights_md, weights_dims,
                                      weights_sizes, data_type,
                                      mkldnn_any));
#if 0
    MKL_CHECK(mkldnn_memory_desc_init(&bias_md, bias_dims,
                                       ...
                                       mkldnn_x));
#endif
    MKL_CHECK(mkldnn_memory_desc_init(&dst_md, dst_dims, dst_sizes,
    ...
    MKL_CHECK(mkldnn_primitive_desc_create(&opkernel->op_desc,
                                           &ip_any_desc, engine, NULL));
    ...
    const_mkldnn_primitive_desc_t kernel_weights_pd = mkldnn_primitive_desc_query_pd(
                                                    opkernel->op_desc,
                                                    mkldnn_query_weights_pd, 0);
    // TODO: what about bias?
    const_mkldnn_primitive_desc_t kernel_dst_pd = mkldnn_primitive_desc_query_pd(
                                                    opkernel->op_desc,
                                                    mkldnn_query_dst_pd, 0);
    // ------------------------------------------------------------------------    
    ...
    if (src_pd) {
        ...
        create_mkldnn_tensor(src_dims, src_sizes, data_type, mkldnn_nc,
                            engine, &(opkernel->inputs[0]));
    }
    ...
    create_mkldnn_tensor_from_pd(dst_dims, dst_sizes, &output_md,
