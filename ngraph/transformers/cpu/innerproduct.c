...
else
{
 ...
 //end bias code 
  ...
  if (src_pd) {
    ...
    create_mkldnn_tensor(src_dims, src_sizes, data_type, mkldnn_oi, engine,
                         &(opkernel->inputs[1]));
  }
  ...
  if (weights_pd) {
    ...
    create_mkldnn_tensor(weights_dims, weights_sizes, data_type, mkldnn_nc,
...
//end Bias code
  ...
  if (!mkldnn_memory_primitive_desc_equal(opkernel->inputs[0].desc,
    ...
    mkldnn_primitive_desc_t reorder_pd;
    MKL_CHECK(mkldnn_reorder_primitive_desc_create(
        &reorder_pd, opkernel->inputs[0].desc, kernel_weights_pd));
    mkldnn_primitive_at_t inputs[] = {
...
//Bias code
  if (!mkldnn_memory_primitive_desc_equal(opkernel->inputs[2].desc,
    ...
    create_mkldnn_tensor_from_pd(bias_dims, bias_sizes, &md, engine,
...
if (bias_sizes)
{
  ...
  ip_srcs[0] = mkldnn_primitive_at(mkldnn_memory_prim_src, 0);
  ip_srcs[1] = mkldnn_primitive_at(mkldnn_memory_prim_weights, 0);
