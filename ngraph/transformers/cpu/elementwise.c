...
mkldnn_netlist_t create_mkldnn_add_kernel(
    ...
    if (src1_pd) {
        ...
        MKL_CHECK(mkldnn_memory_desc_init(&md1, src1_dims, src1_sizes,
                                      data_type, mkldnn_x));
    ...
    if (src2_pd) {
        ...
        MKL_CHECK(mkldnn_memory_desc_init(&md2, src2_dims, src2_sizes,
                                      data_type, mkldnn_x));
    ...
    MKL_CHECK(mkldnn_sum_primitive_desc_create(&opkernel->op_desc, NULL,
                                               num_matrix_to_add, scale_vector,
                                               input_pds));
    ...
    if (src1_pd) {
        mkldnn_memory_desc_t md = *(mkldnn_primitive_desc_query_memory_d(
              (const_mkldnn_primitive_desc_t) src1_pd));
        create_mkldnn_tensor_from_pd(src1_dims, src1_sizes, &md,
                                     engine, &(opkernel->inputs[0]));
    } else {
        create_mkldnn_tensor(src1_dims, src1_sizes, data_type, mkldnn_x,
                             engine, &(opkernel->inputs[0]));
    }
    ...
    if (src2_pd) {
        mkldnn_memory_desc_t md = *(mkldnn_primitive_desc_query_memory_d(
              (const_mkldnn_primitive_desc_t) src2_pd));
        create_mkldnn_tensor_from_pd(src2_dims, src2_sizes, &md,
                                     engine, &(opkernel->inputs[1]));
    } else {
        create_mkldnn_tensor(src2_dims, src2_sizes, data_type, mkldnn_x,
                             engine, &(opkernel->inputs[1]));
    }
