...
from ngraph.op_graph.op_graph import Op, Add, Multiply, Greater, Less
...
class CPUFusion(GraphRewritePass):
    ...
    def construct_relu_fprop_pattern(self):
      ...
      ..
      But we generate only 1 and match_pattern takes care of matching all permutations.
      ...
      x      = ng.PatternLabelOp(self.relu_fwd_x_label)
      max_op = Maximum(x, zero_w_broadcast)
      # We bind slope op to S only if it is scalar.
      slope_label_op = ng.PatternLabelOp(self.relu_fwd_slope_label,
                                 (lambda op: op.is_scalar))
      slope  = ng.PatternSkipOp(slope_label_op,
                                (lambda op: isinstance(op, BroadcastOp)))
    ...
    def fuse_relu_bprop_callback(self, op, label_map_op_list):
      ...
      for (label_map, op) in label_map_op_list:
        ...
        slope = label_map[self.relu_bwd_slope_label]
