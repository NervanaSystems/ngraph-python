...
class FlexFusion(GraphRewritePass):
    ...
    def construct_ce_binary_inner_pattern(self):
        """
        Generate graph op that represents a pattern for Cross Entropy Binary Inner operation
        ng.cross_entropy_binary_inner(y, t, enable_sig_opt=True, enable_diff_opt=True)

        Returns:
            Single pattern that matches "optimized" Cross Entropy Binary Inner
        """
    ...
    def fuse_ce_binary_inner_callback(self, op, label_map_op_list):
        """
        Callback function that handles fusion for cross_entropy_binary_inner pattern
        """
        for (label_map, op) in label_map_op_list:
            # Matched Sigmoid pattern, do the replacement here.
    ...
    def __init__(self):
        ...
        # Register cross_entropy_binary_inner pattern
        pattern_ce_binary_inner = self.construct_ce_binary_inner_pattern()
        self.register_pattern(pattern_ce_binary_inner, self.fuse_ce_binary_inner_callback)
