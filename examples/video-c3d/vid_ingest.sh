...
# Save train config
echo "manifest = [$OUTDIR/test-index.csv, $OUTDIR/train-index.csv]" > $SCRIPT_DIR/train.cfg
...
for setn in train test; do
    ...
    cat $(cat $listfile | sed "s/.avi$/.csv/") |
        ...
        sed "s|^$OUTDIR/\(.*\)/\(.*.avi\)|\1/\2,\1/label.txt|" >> $OUTDIR/${setn}-index-labels.csv
    while read line; do 
        firstpart=$( echo "$line" |cut -d ',' -f 1 )
        filename=$( echo "$line" |cut -d ',' -f 2 )
        label=$(cat $OUTDIR/$filename) 
        echo "$firstpart	$label"  >> $OUTDIR/${setn}-index.csv
    done < $OUTDIR/${setn}-index-labels.csv
    sed -i '1i\'"@FILE	STRING" $OUTDIR/${setn}-index.csv
    #{ echo "@FILE	STRING" ; cat ${setn}-index-labels.csv ; }  > $OUTDIR/${setn}-index.csv
done
