...
from ngraph.frontends.neon import (ax, np, Affine, Conv, Pooling, makeAxes, Callbacks,
...
layers = [Conv((5, 5, 16), init=init_uni, activation=Rectlin(), batch_norm=bn),
          ...
          Affine(nout=10, axes=makeAxes(ax.Y,), init=init_uni, activation=Softmax())]
...
model.initialize(
    dataset=train,
    input_axes=makeAxes((ax.C, ax.D, ax.H, ax.W)),
