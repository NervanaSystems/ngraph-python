...
'''
    Usage:
        python timeseries_lstm.py -t 10000 -b gpu -z 64
...
import numpy as np
import ngraph as ng
from ngraph.frontends.neon import Layer, Sequential, LSTM, Affine, Recurrent
...
data = timeseries.TimeSeries(train_ratio=0.8,  # ratio of samples to set aside for training
                        ...
                        ncycles=3000,     # number of cycles in the curve
                        batch_size=batch_size,
                        curvetype='Lissajous2',
                        predict_seq=predict_seq,  # set to true if you want sequences as output
...
seq1 = Sequential([LSTM(nout=recurrent_units, init=init_uni,
                   activation=Tanh(), gate_activation=Logistic(), return_sequence=predict_seq),
...
if (do_plots is True):
    ...
    # Take only up to 1000 time points
    time_points = min(1000, preds.shape[0])
    ...
    ax.legend()
    ax.grid()
    title = 'Lissajous Curve Predictions and Ground Truth Across Time, Predict Sequence:%s' % predict_seq
    ax.set_title(title)
    fig.savefig('PredictedCurve_Time_Seq_to_Seq_%s.png' % predict_seq, dpi=128)
    plt.clf()
    ...
    ax.set_title('Lissajous Curve Predictions and Ground Truth, 2D Time Series, Predict Sequence:%s' % predict_seq)
    ax.legend()
    ax.grid()
    fig.savefig('PredictedCurve_2D_Seq_to_Seq_%s.png' % predict_seq, dpi=128)
