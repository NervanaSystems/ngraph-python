    ...
    def __init__(self, num_fils, direct=True, strides=1):
        ...
        if en_bottleneck:
            ...
            if(direct):
                ...
                self.side_path = Convolution(**conv_params(1, num_fils * 4,
        ...
        # This is for CIFAR10 and Resnet18 and Resnet34 for i1K
            ...
            if(direct):
                ...
                self.side_path = Convolution(**conv_params(1, num_fils,
                                                           strides=strides, relu=False))
    ...
    def __init__(self, net_type):
        ...
        # For I1K dataset
            ...
            layers = [
                ...
                # First Conv layer
                Convolution((7, 7, 64), rho=rho_val, strides=2, padding=3,
                            batch_norm=True, activation=Rectlin(), filter_init=weight_init),
                # Max Pooling
...
for step, data in enumerate(train_set):
    ...
    if (step + 1) % args.iter_interval == 0 and step > 0:
        ...
        diff = abs(eval_losses['cross_ent_loss'] - (interval_cost / args.iter_interval))
        tqdm.write("Interval {interval} Iteration {iteration} complete. "
                   "Avg Train Cost {cost:0.4f} Test Avg loss:{tcost} Diff:{diff_val}".format(
                       interval=step // args.iter_interval,
                       iteration=step,
                       cost=interval_cost / args.iter_interval, tcost=eval_losses, diff_val=diff))
        # For graph plotting
