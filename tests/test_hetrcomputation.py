...
def check_result_values(input_vector, result_expected, placeholder, op_list=[], *args):
    ...
    """
    This function checks the result values return by the hetr computation object against the expected result values
    it also checks if the value returned by the hetr object matches the order in the expected result list

    :param: input_vector: list specifying the differnt values to be passed to the placeholder
    :param: result_expected: list of tuples specifying the expected result values from the hetr computation object
    :param: placeholder: list of placeholder to be passed for HetrComputation class
    :param: op_list: list of result handlers to be paased for HetrComputation class
    ...
    computation = HetrComputation(transformer, op_list, placeholder)
    result_obtained = []
    ...
    # if return result is  scalar
        ...
        assert (np.array(tuple(result_obtained)) == np.array(result_expected[0])).all() == True  
...
def check_device_assign_pass(default_device, default_device_id,
    ...
    :param: default_device: string, the default device for each op, if not specified by user ex: "numpy"
    :param: default_device_id: string, the default device number for each op, if not specified by user ex: "0"
    :param: graph_op_metadata: dict, dictionary of list specifying  the expected metadata {device_id, device} for each op
...
def check_communication_pass(ops_to_transform, expected_recv_nodes):
    ...
    """
    The communication pass should insert send/recv nodes wherever the metadata[transformer] differs between nodes. 
    This checks that the recv nodes are inserted in the right place, and counts that the expected number of send 
    nodes are found.

    :param ops_to_transform: list of ops to do the garph traversal 
    :param expected_recv_nodes: lits of ops where receive nodes are expected to be inserted after the communication pass 
...
def test_hetr_graph_passes():
    ...
    check_communication_pass(ops_to_transform=graph_op_list, expected_recv_nodes=[x_plus_y])
...
def test_simple_graph():
    ...
    check_result_values(input_vector=[10, 20, 30], result_expected=[(11, 21, 31)],
...
def test_multiple_results():
    ...
    check_result_values(input_vector=[10, 20, 30], result_expected=[(11, 12, 30), (21, 22, 60), (31, 32, 90)],
                       placeholder=x, op_list=[x_plus_one, x_plus_two, x_mul_three])
