...
def test_gemm_multiply_matrices(transformer_factory, rows_1, col_1, col_2, description):
    ...
    :return: PASS if dot product of flex calculations passes assert_allclose comparing with dot product of numpy,
             FAIL if don't. Overflow/underflow doesn't occur for above test cases.
    Those test cases check only autoflex initialisation - check result of dot product for matrices with specific shapes.
    """
...
@pytest.mark.parametrize("row, col, const_val, flex_exceptions, iters, description", (
    (3, 3, 1, [[15.99951171875, 15.99951171875, 15.99951171875]], 3, "Dot product of matrix and positive vector"),
