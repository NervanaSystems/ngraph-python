...
# TODO This is mostly historical, and is not up to date with the current
# transformer.
class GenNumPyTransformer(Transformer):
    ...
    def evaluate(self, **kvargs):
        ...
        for i, op in enumerate(self.ops):
            ...
            val = '{var} = {val} # Live={live}'.format(val=op.evaluate(
                self, varname(op), *args), var=varname(op), live=live)
            vals[op] = val
            body.append(val)
        for line in body:
            print(line)
    ...
    def constant(self, value, axes, dtype):
        return 'constant {dtype} {axes} = {value}'.format(
            value=value, axes=axes, dtype=dtype)
    ...
    def dot(self, x, y, red_axes, out):
        return 'np.dot({x}, {y}, axes={a}, out={out})'.format(
            x=x, y=y, out=out, a=red_axes)
    ...
    def sig(self, x, out):
        ...
        {out}nnp.reciprocal({out}, {out})'.format(x=x, out=out)
    ...
    def softmax(self, x, batch_axes, out):
        return 'softmax({x}, batch_axes={batch_axes}, out=out'.format(
            x=x, batch_axes=batch_axes, out=out)
    ...
    def sum(self, x, reduction_axes, out):
        return 'np.sum({x},axis={a}, out={out})'.format(
            x=x, a=reduction_axes, out=out)
