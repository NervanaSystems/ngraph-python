...
one_inputs_ops = {
    ...
    # temporarily use tanh as TruncatedNormal is not implemented
    'TruncatedNormal': np.random
}
    ...
    for node in graph_def.node:
        ...
        with be.bound_environment(env):
            if op_type in two_inputs_ops:
                if (inputs[0] not in name_to_op or inputs[
                        1] not in name_to_op):
                    continue
                op = two_inputs_ops[op_type](name_to_op[inputs[0]], name_to_op[
                ...
                shape = [d.size for d in dims.dim]
                op = be.placeholder(
                    axes=name_to_axes[node.name], name=node.name)
                if len(shape) == 2:
                    graph.x = op
                elif len(shape) == 1:
                    graph.y = op
                ...
                # TODO: op should be initialized with op = be.Constant(np_val,
                # name=node.name)
                ...
                if len(shape) == 2:
                    if 'weights' in node.name:
                        ...
                        op = be.NumPyTensor(
                            np_val, axes=[in_axis, out_axis], name=node.name)
                        in_axis = out_axis  # now the output axis becomes input axis for the next layer
                    else:
                        op = np_val
                elif len(shape) == 1:
                    if 'biases' in node.name:
                        ...
                        assert(in_axis.length == shape[0])
                        op = be.NumPyTensor(
                            np_val, axes=[in_axis], name=node.name)
                    else:
                        op = np_val
                else:
                    op = np_val
                ...
                op = be.Variable(axes=name_to_axes[
                                 node.name], init=Uniform(-.001, .001), name=node.name)

            elif op_type == 'Cast':
                dst_type = node.attr['DstT']
                src_type = node.attr['SrcT']
                # TODO: currently just use the original format, need a real
                # cast
                op = name_to_op[inputs[0]]
            elif op_type == 'SparseSoftmaxCrossEntropyWithLogits':
                logscale = -np.float(1. / np.log(2.0))
                op = be.sum(be.safelog(name_to_op[
                            inputs[0]]) * name_to_op[inputs[1]], out_axes=(batch_axis,)) * logscale
            elif op_type == 'Mean':
