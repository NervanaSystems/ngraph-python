...
def test_evalutaion_twice():
    ...
    """
    x = be.NumPyTensor(np.array([[1, 2], [3, 4]], dtype='float32'),
                       axes=be.Axes(be.NumericAxis(2),
                                    be.NumericAxis(2)))

    hidden1_weights = be.NumPyTensor(np.array([[1], [1]], dtype='float32'),
                                     axes=be.Axes(be.NumericAxis(2),
                                                  be.NumericAxis(1)))

    hidden1_biases = be.NumPyTensor(np.array([[2], [2]], dtype='float32'),
                                    axes=be.Axes(be.NumericAxis(2),
                                                 be.NumericAxis(1)))
